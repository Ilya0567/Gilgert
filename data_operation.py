...
def check_product(product: str) -> str:
    ...
    if in_recommended and not in_avoid:
        ...
        return "Этот продукт есть как в списке рекомендованных, так и в списке запрещённых. Поэтому вопрос передан нашим специалистам, от которых я скоро принесу Вам ответ."
    else:
        return "Я не нашел информацию по этому продукту, поэтому передал вопрос нашим специалистам, от которых я скоро принесу Вам ответ."
